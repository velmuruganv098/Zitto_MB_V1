...
#define UART_BAUD               500000   /* V0.0073: must match S32K UART_BAUDRATE (was 115200) */
#define BLE_BATCH_MS            15       /* CAN lines are packed into one notify for up to 15 ms */
...
/*
 * V0.0073 BLE output queue.
 *
 * publish() is called from two contexts: loop() (decoded UART frames) and the
 * NimBLE host task (BLE write callback: CMD_SENT / PONG / ...).  Calling
 * setValue()+notify() on the same characteristic from both at once lost
 * notifications on the bench (a command's result LOG line reached COM5 but
 * never the client).  Now publish() only appends "line\n" to a queue under a
 * spinlock; bleTxTask() - loop() only - packs queued lines into MTU-sized
 * notifications.  Urgent lines (everything except CAN data) are sent on the
 * next loop() pass; CAN lines wait up to BLE_BATCH_MS so several share one
 * notification.  VCU Master and the mobile app split notifications on '\n'.
 */
#define BLE_Q_SIZE 8192
static char     g_bleQ[BLE_Q_SIZE];
static uint16_t g_bleQHead = 0;           /* write index */
static uint16_t g_bleQTail = 0;           /* read index  */
static uint32_t g_bleQOldestMs = 0;
static bool     g_bleQUrgent = false;
static uint32_t g_bleQDropped = 0;
static uint32_t g_bleNotifies = 0;
static uint32_t g_bleLines = 0;
static uint32_t g_canFramesRx = 0;
static portMUX_TYPE g_bleQMux = portMUX_INITIALIZER_UNLOCKED;

static uint16_t bleQUsed()
{
    return (uint16_t)((g_bleQHead + BLE_Q_SIZE - g_bleQTail) % BLE_Q_SIZE);
}

static uint16_t bleBatchMax()
{
    uint16_t mtu = BLEDevice::getMTU();
    uint16_t n = (mtu > 3) ? (uint16_t)(mtu - 3) : 20;
    if (n > 500) n = 500;
    return n;
}

static void bleEnqueue(const String &out, bool urgent)
{
    uint16_t n = (uint16_t)out.length();
    portENTER_CRITICAL(&g_bleQMux);
    if ((uint32_t)bleQUsed() + n + 1 >= BLE_Q_SIZE)
    {
        g_bleQDropped++;
    }
    else
    {
        if (bleQUsed() == 0)
        {
            g_bleQOldestMs = millis();
        }
        for (uint16_t k = 0; k < n; k++)
        {
            g_bleQ[g_bleQHead] = out[k];
            g_bleQHead = (uint16_t)((g_bleQHead + 1) % BLE_Q_SIZE);
        }
        g_bleQ[g_bleQHead] = '\n';
        g_bleQHead = (uint16_t)((g_bleQHead + 1) % BLE_Q_SIZE);
        g_bleLines++;
    ...
    portEXIT_CRITICAL(&g_bleQMux);
}

/* Set by the TX characteristic's onStatus() callback, which NimBLE calls
 * synchronously from notify(): lets bleTxTask() see a failed notify (host
 * out of buffers under bursts) and keep the chunk queued for a retry. */
static volatile uint32_t g_notifyFailCount = 0;
static uint32_t g_notifyRetryAtMs = 0;

/* loop() only: send queued lines, packed on line boundaries.  A chunk is
 * removed from the queue only after notify() succeeded. */
static void bleTxTask()
{
    static char chunk[512];
    uint16_t maxLen, used, len, lastNl, k;

    if (!g_bleConnected || g_txChar == nullptr)
    {
        portENTER_CRITICAL(&g_bleQMux);
        g_bleQTail = g_bleQHead;
        g_bleQUrgent = false;
        portEXIT_CRITICAL(&g_bleQMux);
        return;
    }
    if ((int32_t)(millis() - g_notifyRetryAtMs) < 0)
    {
        return;                                      /* back off after a failed notify */
    }
    maxLen = bleBatchMax();
    for (int rounds = 0; rounds < 4; rounds++)
    {
        portENTER_CRITICAL(&g_bleQMux);
        used = bleQUsed();
        bool due = (used >= maxLen) || g_bleQUrgent ||
                   (used != 0 && (millis() - g_bleQOldestMs) >= BLE_BATCH_MS);
        if (used == 0 || !due)
        {
            if (used == 0) g_bleQUrgent = false;
            portEXIT_CRITICAL(&g_bleQMux);
            return;
        }
        len = (used < maxLen) ? used : maxLen;
        lastNl = 0;
        for (k = 0; k < len; k++)
        {
            chunk[k] = g_bleQ[(g_bleQTail + k) % BLE_Q_SIZE];
            if (chunk[k] == '\n') lastNl = (uint16_t)(k + 1);
        }
        if (lastNl == 0)
        {
            lastNl = len;                    /* single over-long line: send a slice */
        }
        portEXIT_CRITICAL(&g_bleQMux);

        uint32_t failBefore = g_notifyFailCount;
        g_txChar->setValue((uint8_t *)chunk, lastNl);
        g_txChar->notify();
        if (g_notifyFailCount != failBefore)
        {
            g_notifyRetryAtMs = millis() + 3;        /* keep the chunk, retry shortly */
            return;
        }
        g_bleNotifies++;

        portENTER_CRITICAL(&g_bleQMux);
        g_bleQTail = (uint16_t)((g_bleQTail + lastNl) % BLE_Q_SIZE);
        if (bleQUsed() == 0)
        {
            g_bleQUrgent = false;
        }
    ...
    uint16_t len)
{
    ...
    switch (type)
    {
        ...
        case MSG_IMU:
        {
            ...
            /* V0.0073 motion tracking fields (64-byte packet) */
            if (len >= 64)
            {
                s += " pos_mm=(";
                s += String(rdI32(p + 32) / 10.0f, 1);
                s += ",";
                s += String(rdI32(p + 36) / 10.0f, 1);
                s += ",";
                s += String(rdI32(p + 40) / 10.0f, 1);
                s += ") dist_mm=";
                s += String(rdU32(p + 44) / 10.0f, 1);
                s += " rpy_deg=(";
                s += String(rdI16(p + 48) / 100.0f, 1);
                s += ",";
                s += String(rdI16(p + 50) / 100.0f, 1);
                s += ",";
                s += String(rdI16(p + 52) / 100.0f, 1);
                s += ") moving=";
                s += String(p[54]);
                s += " imu_flags=";
                s += String(p[55]);
            ...
            if (g_crcRx == g_crcCalc)
            {
                ...
                publish(
                    decodeFrame(
                        ...
                        g_payload,
                        g_len),
                    true,               /* V0.0073: MSG_LOG now carries S32K command/action events
                                           (RTT debug mirror is off by default) - forward to BLE */
                    g_type == MSG_CAN);
            }
            else
            {
...
{
    publish(
        "INFO Zitto_MB_V1_Bridge UART2=" + String(UART_BAUD) + " BLE=ON RAW=1 BATCH=1 FW=VCUMASTER");
}
...
static void printStats()
{
    ...
    s += " can_frames=";
    s += String(g_canFramesRx);

    s += " ble_lines=";
    s += String(g_bleLines);

    s += " ble_notifies=";
    s += String(g_bleNotifies);

    s += " ble_q_drop=";
    s += String(g_bleQDropped);
